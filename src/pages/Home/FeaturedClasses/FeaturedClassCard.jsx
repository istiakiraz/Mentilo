...
const FeaturedClassCard = ({ classCard }) => {
  return (
        ...
        <div className=" grid grid-cols-6 ">
          <span className="col-span-4">
            ...
            <h3>
              ...
              {classCard.trainers.length*Math.floor(Math.random() * 9 + 10)}
            </h3>
            <h3 className="text-sm w-11/12 hyphens-auto overflow-x-hidden mt-2" > <span className="font-bold">Description :</span> {classCard.description}</h3>
          </span>

          <div className="grid grid-cols-3 col-span-2 w-full gap-2 items-center ">
            ...
            {classCard.trainers.map((trainer, index) => (
              <Link to={`/trainer-details/${trainer._id}`} key={index}>
                <img
                  className="size-8 object-cover border-2 border-primary rounded-full"
                  src={trainer.photo}
                  alt={trainer.email}
                />
              </Link>
