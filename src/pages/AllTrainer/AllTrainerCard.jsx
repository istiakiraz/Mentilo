  ...
  return (
    <div className="bg-secondary shadow-[5px_5px_0px_0px_#432365]  pb-6 rounded-2xl ">
      ...
      <div className="px-6 pt-3">
        <div className="flex flex-wrap items-center justify-between">
          ...
          <span className="flex gap-1 items-center">
            <Link target="_blank" to={trainer.facebook}>
              <IoLogoFacebook size={20} color="#432365" />
            </Link>
            <Link target="_blank" to={trainer.instagram}>
              <FaInstagram size={20} color="#432365" />
            </Link>
          </span>
        </div>
        ...
        <p className="flex items-center mt-1 gap-2">
          ...
          {trainer.skills.map((skill, index) => (
            <p
              className="text-sm cursor-pointer hover:bg-primary/70 duration-300 bg-primary/50 text-white px-2 rounded-2xl border border-primary"
              key={index}
            >
              {skill}
            </p>
      ...
      <Link
        ...
        className="px-6 flex -mt-4 justify-end "
      >
